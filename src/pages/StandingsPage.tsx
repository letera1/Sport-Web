  ...
  if (error) {
    return (
      <div className="flex flex-col gap-4">
        <h1 className="text-xl sm:text-2xl font-semibold text-text-primary">Standings</h1>
    ...
    <div className="flex flex-col gap-4 sm:gap-6">
      ...
      <div className="bg-surface rounded-xl overflow-hidden border border-divider/30">
        ...
        {!loading && standings.length > 0 && (
          <div className="divide-y divide-divider/20 stagger-children">
            {standings.map((row) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "grid grid-cols-[2.5rem_1fr_2rem_2rem_2rem_2rem_2.5rem_2.5rem_2.5rem_3rem] sm:grid-cols-[3rem_1fr_2.5rem_2.5rem_2.5rem_2.5rem_3rem_3rem_3rem_3.5rem_5rem] gap-1 sm:gap-2 px-3 sm:px-4 py-2.5 items-center cursor-pointer hover:bg-surface-hover transition-colors text-xs sm:text-sm",
                    "border-l-[3px]",
                    isChampionsLeague ? "border-l-accent" :
                    isEuropaLeague ? "border-l-info" :
                    isRelegation ? "border-l-danger" :
                ...
                >
                  ...
                  <span className="text-center text-text-secondary">{row.intWin}</span>
                  <span className="text-center text-text-secondary">{row.intDraw}</span>
                  <span className="text-center text-text-secondary">{row.intLoss}</span>
                  <span className="text-center text-text-secondary">{row.intGoalsFor}</span>
                  <span className="text-center text-text-secondary">{row.intGoalsAgainst}</span>
                  <span className={cn(
                  ...
                  <span className="text-center font-bold text-text-primary">{row.intPoints}</span>
